import{J as b,t as i,e as a,i as u,n as x,K as w,L as S,l as q,M as p,N as f,O as E}from"./index-wF9wK4Yp.js";const L=d=>document.getElementById(d),m=()=>L("screens"),C={tide:"🌊",garden:"🌱",stump:"🥥",vines:"🍇"},$=3;function v(d){return m().innerHTML=`<div class="screen opaque">${d}</div>`,m().firstElementChild}function _(d,s,t=$){const l=[];for(let r=0;r<t;r++){const o=S(),e=new x(d*31+r*7+1),n=[];for(let c=0;c<q.problemsPerChamber;c++){const k=c>0?{world:s,rng:e,kind:n[0].kind}:{world:s,rng:e};let h=p(f(o,k),o,{world:s,rng:e});c>0&&h.kind!==n[0].kind&&(h=p(f(o,{world:s,rng:e,kind:"fetch",skill:h.skillId}),o,{world:s,rng:e})),n.push(h),E(o,h,{correct:!0,usedHint:!1,ms:4e3})}l.push(n)}return l}class g{constructor(s,t,l,r,{soloChallenge:o=!1,code:e=null}={}){this.game=s,this.world=l,this.seed=r,this.code=e,this.solo=o,this.players=t.map(n=>({id:n,name:b().find(c=>c.id===n)?.name||"?",score:0})),this.roundsProblems=_(r,l),this.round=0,this.turn=0,this.queue=[],this.rng=new x(r^24301)}start(){this._interstitial()}_interstitial(){const s=this.players[this.turn];v(`
    ...
    `).querySelector("#duel-go").addEventListener("click",()=>{u.sfx("click"),this._playTurn()})}_playTurn(){const s=this.players[this.turn];this.queue=this.roundsProblems[this.round].slice();const t=this.game;t.profile=w(s.id),t.duel=this,t.currentWorld=this.world,t.combo=0,t.chamberIndex=this.round,t.refreshHudCounts(),t.runChamber()}nextProblem(){return this.queue.shift()}hasMore(){return this.queue.length>0}scoreCorrect(s){const t=this.players[this.turn];t.score+=10+Math.min(10,(s-1)*2)}scoreWrong(){}chamberDone(){const s=this.game;if(s.duel=null,!this.solo&&this.turn<this.players.length-1){this.turn++;const t=this.players[this.turn];v(`
      ...
      `).querySelector("#duel-go").addEventListener("click",()=>this._interstitial());return}if(this.turn=0,this.round++,this.round<this.roundsProblems.length){this._interstitial();return}this._finish()}_finish(){const s=this.game;let t;if(this.solo){const r=this.players[0];t=`
        ...
        <div style="flex:2"></div>`}v(t).querySelector("#duel-done").addEventListener("click",()=>s.showTitle())}}function y(d,s){return`${{tide:"T",garden:"G",stump:"S",vines:"V"}[d]||"G"}${$}-${s.toString(36).toUpperCase()}`}function z(d){const s=String(d).trim().toUpperCase().match(/^([TGSV])(\d)-([0-9A-Z]+)$/);if(!s)return null;const t={T:"tide",G:"garden",S:"stump",V:"vines"}[s[1]],l=parseInt(s[3],36);return!t||!Number.isFinite(l)?null:{world:t,seed:l}}function M(d){const s=b();let t=[],l="garden";const r=()=>{const o=v(`
      ...
      <div class="card">
        <div class="menu-row">
          ${Object.entries(C).map(([e,n])=>`<button class="btn soft" data-world="${e}" ${e===l?'style="outline:4px solid var(--sun)"':""}>${n} ${i("world."+e)}</button>`).join("")}
        </div>
      </div>
      <div class="menu-row">
