import{$ as b,t as i,e as c,a0 as w,j as u,n as x,a1 as S,a2 as q,k as E,a3 as p,a4 as f,a5 as C}from"./index-C1dXyLKD.js";const L=d=>document.getElementById(d),m=()=>L("screens"),$=3;function v(d){return m().innerHTML=`<div class="screen opaque">${d}</div>`,m().firstElementChild}function _(d,s,t=$){const l=[];for(let r=0;r<t;r++){const o=q(),e=new x(d*31+r*7+1),n=[];for(let a=0;a<E.problemsPerChamber;a++){const k=a>0?{world:s,rng:e,kind:n[0].kind}:{world:s,rng:e};let h=p(f(o,k),o,{world:s,rng:e});a>0&&h.kind!==n[0].kind&&(h=p(f(o,{world:s,rng:e,kind:"fetch",skill:h.skillId}),o,{world:s,rng:e})),n.push(h),C(o,h,{correct:!0,usedHint:!1,ms:4e3})}l.push(n)}return l}class g{constructor(s,t,l,r,{soloChallenge:o=!1,code:e=null}={}){this.game=s,this.world=l,this.seed=r,this.code=e,this.solo=o,this.players=t.map(n=>({id:n,name:b().find(a=>a.id===n)?.name||"?",score:0})),this.roundsProblems=_(r,l),this.round=0,this.turn=0,this.queue=[],this.rng=new x(r^24301)}start(){this._interstitial()}_interstitial(){const s=this.players[this.turn];v(`
    ...
    `).querySelector("#duel-go").addEventListener("click",()=>{u.sfx("click"),this._playTurn()})}_playTurn(){const s=this.players[this.turn];this.queue=this.roundsProblems[this.round].slice();const t=this.game;t.profile=S(s.id),t.duel=this,t.currentWorld=this.world,t.combo=0,t.chamberIndex=this.round,t.refreshHudCounts(),t.runChamber()}nextProblem(){return this.queue.shift()}hasMore(){return this.queue.length>0}scoreCorrect(s){const t=this.players[this.turn];t.score+=10+Math.min(10,(s-1)*2)}scoreWrong(){}chamberDone(){const s=this.game;if(s.duel=null,!this.solo&&this.turn<this.players.length-1){this.turn++;const t=this.players[this.turn];v(`
      ...
      <div class="card">
        <div class="menu-row">
          ${Object.entries(w).map(([e,n])=>`<button class="btn soft" data-world="${e}" ${e===l?'style="outline:4px solid var(--sun)"':""}>${n} ${i("world."+e)}</button>`).join("")}
        </div>
      </div>
      <div class="menu-row">
